...
//
// ⚠️  CHANTIER EN ATTENTE — voir échange avec Guillaume (session du 8 août) : "à chaque
// fois qu'on traite ces cas particuliers, ça se retranscrit mal de manière générique pour
// les autres cas analogues qui seront rencontrés ultérieurement". CONSTAT : ce moteur n'a
// aucune fonction centralisée pour des concepts récurrents — chaque fonction réimplémente
// sa propre version locale, avec ses propres angles morts. Trouvé et corrigé séparément,
// PLUSIEURS FOIS, dans des endroits différents cette seule session :
//   - "Est-ce un cue-bid (couleur de l'adversaire, pas une couleur à soi) ?" — au moins
//     3 endroits distincts, dont l'un encore non résolu (voir README, "contres
//     compétitifs en 4ème main").
//   - "Le partenaire a-t-il déjà tout dit / conclu, plus rien à ajouter ?" — au moins 5
//     versions quasi-identiques reconstruites à la main (2SA fitté, cue-bid accepté,
//     transfert Texas accepté, redemande 1SA + retour simple, reconvert accepté).
//   - "Est-ce une VRAIE ouverture (précédée de passes) ou une intervention ?" — corrigé à
//     un seul endroit (donne 5, session du 8 août), jamais vérifié ailleurs.
// À FAIRE dans une session DÉDIÉE (jamais à la volée en cours de traitement d'une donne) :
// cartographier tous les points d'usage actuels de ces concepts, construire des fonctions
// de reconnaissance centralisées (ex. isCueBid(bid, history, seat), isPartnerAlready
// Committed(history, seat), wasRealOpening(entry, history)), puis les substituer partout
// où une logique équivalente a été réinventée localement — plutôt que de continuer à
// patcher au cas par cas. Rappeler ce chantier à Guillaume à chaque nouvelle sollicitation
...
function decideRobotOpening(hand, hcp, hl, dealVulnerable, seat) {
    ...
    if (hl >= barrageFloor && hl <= 12) {
        ...
        // Voir échange avec Guillaume (vraie donne testée, "il est interdit d'ouvrir
        // d'un 2 majeur faible avec 4+ cartes dans l'autre majeure") : garde manquante
        // — un barrage 2-majeur ne doit jamais se faire si l'AUTRE majeure a 4+ cartes
        // (risque de rater un vrai bicolore, ou de gêner sa propre description).
        const otherMajor = longest === 'S' ? 'H' : 'S';
        const otherMajorTooLong = isMajor && lengths[otherMajor] >= 4;
        if (isMajor && len === 6 && !otherMajorTooLong) return '2' + longest;
...
// manche également.
// Voir échange avec Guillaume (vraie donne testée, "il faut implémenter les
// interventions sur 1SA, qui ne sont pas pareilles que les autres", fiche
// C-1SA-X-Q-corr-1 intégralement) : système NATUREL complet après un contre
// (traité comme punitif — cas le plus fréquent selon la fiche elle-même,
// "aujourd'hui... préfère lui donner le sens de mineure-majeure" pour un
// contre DIRECT sur l'ouverture, mais "la défense ci-dessous ne s'applique
// que si le contre est punitif" ; en l'absence de conventions du camp adverse
// alertées, PUNITIF reste l'hypothèse par défaut) — Stayman et Texas
// abandonnés, chaque couleur devient naturelle et non forcing.
function decideResponseToDoubledOneNT(hand, hcp, hl, seat, history) {
    const lengths = suitLengths(hand);
    const longSuits = ['S', 'H', 'D', 'C'].filter(s => lengths[s] >= 5)
        .sort((a, b) => lengths[b] - lengths[a]);

    // Voir échange avec Guillaume (vraie donne testée) : XX est réservé aux mains SANS
    // couleur 5ème (pendant du Passe, "sans couleur au moins 5ème") — avec une vraie
    // longue, la montrer prime toujours, même si le HL (gonflé par la longueur elle-
    // même) dépasse légèrement le seuil de 7 indiqué pour les entrées de couleur.
    if (longSuits.length > 0 && hcp <= 10) {
        const bestSuit = longSuits[0];
        const len = lengths[bestSuit];
        if (len >= 5 && len <= 6) {
            const c = '2' + bestSuit;
            if (isCallLegal(history, c, seat)) return c;
        }
    ...
    // XX : par définition punitif, systématique dès 8HL, non forcing — réservé aux
    // mains sans couleur 5ème qualifiante (voir ci-dessus).
    if (hl >= 8) {
        if (isCallLegal(history, 'XX', seat)) return 'XX';
    }

    // 2SA conventionnel : 6HL+, 5+ cartes dans CHAQUE mineure.
    if (hl >= 6 && lengths['C'] >= 5 && lengths['D'] >= 5) {
        if (isCallLegal(history, '2NT', seat)) return '2NT';
    }

    // Passe : jusqu'à 7HL, sans meilleure enchère, sans couleur 5ème.
    return 'PASS';
}
...
function decideRobotResponse(hand, hcp, hl, partnerCall, seat, history, partnerPromises5Plus, partnerWasIntervening, partnerBidWasOpening, partnerBidWasReopening) {
    ...
    // Voir échange avec Guillaume (vraie donne testée, "Nord répond avec 4HL... il n'a
    // pas plus de raison de dire 2♣ que 1♣", fiche C-1T-1K-Q-corr-1, "Passe 0-5HL") :
    // en contexte compétitif, les points de soutien calculés (bonus fit+distribution)
    // peuvent gonfler artificiellement une main faible au-delà du seuil — plancher
    // absolu sur le HL réel (pas le calcul gonflé), conforme au texte exact de la fiche.
    const belowAbsoluteFloorInCompetitive = partnerWasIntervening && hl < 6 && lengths[suit] < 6;
    if (lengths[suit] >= 3 && supportPoints >= 6 && !majorAlreadyDeclinedByDesign && !minorAlreadyDeclinedByDesign
            && !belowAbsoluteFloorInCompetitive) {
...
function estimateAuctionSideMinPoints(history, side) {
    ...
    const openingParsedForMin = parseBid(opening.call);
    // Voir échange avec Guillaume (vraie donne testée, "réveil absurde de Sud" — la
    // donne 8) : bug trouvé — cette fonction supposait toujours un plancher
    // d'ouverture standard (12H), même quand l'ouverture était un 2♣/2♦ fort (CRM),
    // dont le plancher réel est bien plus haut (16H minimum, souvent 22H+). Sans
    // cette distinction, la force du camp adverse était largement sous-estimée,
    // ce qui ne bloquait pas un réveil qui n'avait pourtant aucun sens.
    if (openingParsedForMin && openingParsedForMin.level === 2
            && (openingParsedForMin.strain === 'C' || openingParsedForMin.strain === 'D')) {
        minPoints = 16;
    }
    ...
    if (sideBids.length >= 2 && sideBids[1].seat !== opening.seat) {
        ...
        // Voir échange avec Guillaume (vraie donne testée, "encore un réveil absurde en
        // Sud") : un vrai 2/1 (palier 2+, nouvelle couleur, pas SA) promet 13+ (déjà
        // confirmé cette session — "après un 2/1, forcing de manche"), pas le plancher
        // générique "+6" qui sous-estimait largement la force adverse ici.
        const openingWasSuitForMinPoints = openingParsedForMin && openingParsedForMin.strain !== 'NT' && openingParsedForMin.level === 1;
        const wasGenuine2Over1ForMinPoints = openingWasSuitForMinPoints && secondParsed && secondParsed.level >= 2
            && secondParsed.strain !== openingParsedForMin.strain && secondParsed.strain !== 'NT';
        ...
        } else if (wasGenuine2Over1ForMinPoints) {
            minPoints = Math.max(minPoints, 13 + 12); // 2/1 forcing de manche : 13+ chez le répondant, combiné au plancher d'ouverture
        } else {
...
function decideRobotIntervention(hand, hcp, hl, seat, history, dealVulnerable, isReopening) {
    ...
    // Voir échange avec Guillaume (vraie donne testée, "pourquoi Est dit 2♣ ?", fiche
    // C-1T-P-1P-2T, "cue-bid de l'ouvreur 'en sandwich', indique conventionnellement un
    // bicolore [rouge]") : en position "en sandwich" (adversaire a ouvert une mineure,
    // SON PARTENAIRE a déjà répondu naturellement), cette mineure d'ouverture devient
    // conventionnellement réservée au cue-bid — jamais annonçable naturellement, même
    // avec une vraie longueur personnelle là. Sans cette main précise (bicolore rouge
    // 5+5+), ni assez pour le contre d'appel (17HL+ ici) : passe.
    const openingEntryForSandwich = history.find(e => isBidCall(e.call));
    const openingParsedForSandwich = openingEntryForSandwich ? parseBid(openingEntryForSandwich.call) : null;
    const isSandwichPosition = openingEntryForSandwich && openingParsedForSandwich
        && (openingParsedForSandwich.strain === 'C' || openingParsedForSandwich.strain === 'D')
        && openingParsedForSandwich.level === 1
        && partnershipOf(openingEntryForSandwich.seat) !== partnershipOf(seat)
        && history.slice(0, history.indexOf(openingEntryForSandwich)).every(e => isPass(e.call))
        && history.some(e => e !== openingEntryForSandwich && isBidCall(e.call)
            && partnershipOf(e.seat) === partnershipOf(openingEntryForSandwich.seat)
            && parseBid(e.call) && parseBid(e.call).strain !== openingParsedForSandwich.strain
            && parseBid(e.call).strain !== 'NT');
    const suit = longestSuitPreferHigh(lengths);
    if (isSandwichPosition && suit === openingParsedForSandwich.strain) return 'PASS';
...
function decideOpenerRebidAfterNewSuit(hand, hcp, hl, myBid, partnerParsed, seat, history, opponentIntervened) {
    ...
    if (secondSuit) {
        ...
        // Voir échange avec Guillaume (vraie donne testée, "pourquoi Nord dit 3♠ ?", fiche
        // S-1C-2K-Q, "3♠ [après 1♥-2♦] = Splinter, courte à ♠, fit ♦" / "pour montrer un
        // bicolore cher, il suffit de dire 2♠") : cette règle de saut, construite pour un
        // AUTRE contexte (répondant montrant SON propre bicolore après 1x-1y), entre en
        // collision ici — après un vrai 2/1 du partenaire (forcing de manche déjà établi),
        // sauter au palier 3 dans une nouvelle couleur a un sens CONVENTIONNEL DIFFÉRENT
        // (splinter), jamais un simple bicolore. Exclue dans ce contexte précis.
        const partnerWasGenuine2Over1ForBicoloreJump = partnerParsed && partnerParsed.level === 2 && partnerParsed.strain === 'D'
            && myBid.level === 1 && (myBid.strain === 'S' || myBid.strain === 'H');
        if (hl >= 20 && !partnerWasGenuine2Over1ForBicoloreJump) {
...
function decideRobotOpenerRebid(hand, hcp, hl, myOpeningCall, partnerCall, seat, history, opponentIntervened) {
    ...
    if (myBid.level === 1 && myBid.strain !== 'NT' && partnerParsed && partnerParsed.strain === 'NT' && partnerParsed.level === 3) {
    ...
    if (myBid.strain === 'S' || myBid.strain === 'H') {
        if (partnerCall === '2NT') {
            // Voir échange avec Guillaume (vraie donne testée, "Nord devrait conclure à
            // la manche avec tous ces HLD. Pourquoi ne le fait-il pas ?") : bug trouvé —
            // ce seuil comparait le HCP BRUT (13) à 14, ignorant TOTALEMENT la
            // distribution (ici une chicane trèfle, qui devient précieuse dès que le
            // fit à 3 cartes est confirmé par le 2SA du partenaire — ruffs potentiels).
            // Remplacé par un vrai calcul de points de soutien (HCP + longueur +
            // distribution, fit connu à 3 cartes), cohérent avec ce qui est fait
            // partout ailleurs dans ce moteur pour un fit chiffré.
            const supportPointsForFittedTwoNT = computeSupportPoints(hand, myBid.strain, 3);
            // 12-13 points de soutien (mini) : accepte le fit au palier minimal (3) ;
            // 14+ : la manche est acquise (12+ garanti côté partenaire, 14+12=26+).
            const call = (supportPointsForFittedTwoNT >= 14 ? '4' : '3') + myBid.strain;
            if (isCallLegal(history, call, seat)) return call;
        }
        if (partnerCall === '3NT') {
    ...
    if (isRaiseOfMySuit) {
        ...
        // Voir échange avec Guillaume (vraie donne testée, "Nord doit dire 4♠", fiche
        // S-1P-3P-Q, "4♠ NF, main quelconque, 15-21HLD" / "Passe NF, 14-15HLD, rare") :
        // acceptation manquante d'un soutien à SAUT (limite, 4+ cartes garanties, pas 3
        // comme un soutien simple) — le repli "loi des atouts" plus bas suppose à tort
        // 3 cartes garanties, sous-estimant le fit réel. Avec 15HL+, j'accepte
        // directement en manche (la frontière 14-15 chevauche textuellement les 2
        // entrées de la fiche, mais 4♠ est le choix standard à 15, Passe restant
        // l'exception rare réservée au match par paires).
        if (myBid.level === 1 && partnerRaiseWasJumpForSlam && hl >= 15) {
            const call = '4' + myBid.strain;
            if (isCallLegal(history, call, seat)) return call;
        }
        if (supportPoints < 18) {
    ...
    if (partnerParsed) {
        // Voir échange avec Guillaume (vraie donne testée, "Que veut dire le 3♣ de
        // Sud ?") : bug trouvé — ce filet de repli cherchait une 2ème couleur sans
        // jamais exclure celle de l'ADVERSAIRE, produisant une couleur qui SERA lue
        // comme un cue-bid par le partenaire (principe déjà établi ailleurs dans ce
        // moteur : "un cue-bid dans une couleur adverse est systématiquement un
        // cue-bid, jamais une couleur à soi"), pas comme naturelle — alors même que le
        // code pensait montrer un vrai bicolore. Exclusion ajoutée.
        const opponentSuitsForRebidFallback = new Set();
        history.forEach(e => {
            if (isBidCall(e.call) && partnershipOf(e.seat) !== partnershipOf(seat)) {
                const p = parseBid(e.call);
                if (p && p.strain !== 'NT') opponentSuitsForRebidFallback.add(p.strain);
            }
        });
        const order = ['S', 'H', 'D', 'C'].filter(s => s !== myBid.strain && s !== partnerParsed.strain
            && !opponentSuitsForRebidFallback.has(s));
        let secondSuit = null;
    ...
    // Voir échange avec Guillaume (même vraie donne, "il a l'arrêt, il devrait le
    // faire à SA, donc dire 2SA") : avec un vrai arrêt (2 gros honneurs) dans la
    // couleur où l'adversaire est intervenu, et rien de mieux à dire (bicolore
    // bloqué ou absent), montrer cet arrêt à SA — au palier le plus bas légal, qui
    // devient naturellement une invite (pas un forcing de manche direct, le
    // partenaire ayant pu être limité par son intervention adverse à décrire).
    const lastOpponentSuitBid = history.slice().reverse().find(e => isBidCall(e.call) && partnershipOf(e.seat) !== partnershipOf(seat));
    if (lastOpponentSuitBid) {
        const oppParsed = parseBid(lastOpponentSuitBid.call);
        if (oppParsed && oppParsed.strain !== 'NT') {
            const oppSuitCards = hand[oppParsed.strain] || '';
            const oppSuitLen = oppSuitCards.length;
            const hasStopperForRebid = oppSuitCards.includes('A')
                || (oppSuitCards.includes('K') && oppSuitLen >= 2)
                || (oppSuitCards.includes('Q') && oppSuitLen >= 3);
            if (hasStopperForRebid) {
                for (let level = 1; level <= 7; level++) {
                    const c = level + 'NT';
                    if (isCallLegal(history, c, seat)) return c;
                }
...
function decideRobotCall(seat, deal, history) {
    ...
    if (myBids.length === 0) {
            ...
            if (isBidCall(e0.call) && isPass(e1.call) && isBidCall(e2.call) && isBidCall(e3.call)
                ...
                if (openingBidForCuebidResp && responseBidForCuebidResp && partnerCuebid
                    ...
                    if (remainingSuitsForCuebidResp.length === 2) {
                        // Voir échange avec Guillaume (vraie donne testée, "Ouest étant
                        // libéré de parole par la nouvelle enchère de Sud, il devrait
                        // juste passer. Mais s'il avait dû parler, il aurait appliqué la
                        // règle habituelle") : libération d'obligation manquante — un
                        // adversaire reparlant depuis le cue-bid du partenaire (ici South
                        // a rebid "2♠") libère de l'obligation de répondre au cue-bid,
                        // même principe déjà appliqué ailleurs pour d'autres contextes
                        // forcing cette session.
                        const opponentSpokeSinceCuebid = history.slice(cuebidIdxForCuebidResp + 1)
                            .some(e => isBidCall(e.call) && partnershipOf(e.seat) !== partnershipOf(seat));
                        if (!opponentSpokeSinceCuebid || hcp >= 6) {
        ...
        if (partnerJustDoubled) {
            ...
            if (!lastBid) {
                ...
                if (!isMichaelOverMinorForGuard && call !== 'X' && myPartnerBid !== lastBid && isCallLegal(history, 'X', seat)) {
                    ...
                    // Voir échange avec Guillaume (vraie donne testée, "Pourquoi Ouest ne
                    // fait pas un cue-bid ?") : le cue-bid (fit 4+ dans la majeure du
                    // partenaire, 13+ points de soutien) existait déjà dans
                    // decideRobotResponse, mais cette branche du dispatch principal
                    // (decideRobotCall) ne l'appelait jamais — sa propre logique "nouvelle
                    // couleur directe" prenait la main avant, sans jamais considérer le
                    // cue-bid. Vérifié en priorité absolue, avant "nouvelle couleur directe".
                    if (partnerBidInfo && (partnerBidInfo.strain === 'S' || partnerBidInfo.strain === 'H')
                            && lengthsForNegDouble[partnerBidInfo.strain] >= 4) {
                        const supportPointsForOuterCuebid = computeSupportPoints(hand, partnerBidInfo.strain, 5);
                        if (supportPointsForOuterCuebid >= 13) {
                            for (let level = 2; level <= 7; level++) {
                                const c = level + opponentOvercallSuitForNegDouble;
                                if (isCallLegal(history, c, seat)) {
                                    call = c;
                                    explanation = `Cue-bid montrant le fit (${lengthsForNegDouble[partnerBidInfo.strain]} cartes, ${supportPointsForOuterCuebid} points de soutien) — forcing de manche (${points})`;
                                    break;
                                }
                            }
                        }
                    ...
                    const directSuitCandidate = call === 'PASS' && (unshownSuitsForNegDouble.find(s => lengthsForNegDouble[s] >= directSuitMinLength) && hl >= directSuitMinHl)
                ...
                const veryLastEntryForDoubledNT = history[history.length - 1];
                ...
                } else if (partnerBidWasOpening && partnerBidInfo && partnerBidInfo.strain === 'NT' && partnerBidInfo.level === 1
                        && veryLastEntryForDoubledNT.call === 'X' && veryLastEntryForDoubledNT === history[partnerBidIndex + 1]) {
                    // Voir échange avec Guillaume (vraie donne testée, "il faut
                    // implémenter les interventions sur 1SA, qui ne sont pas pareilles
                    // que les autres", fiche C-1SA-X-Q-corr-1) : après un contre
                    // (traité comme punitif, cas le plus simple et le plus fréquent
                    // selon la fiche) du 1SA d'ouverture du partenaire, TOUT le système
                    // change — Stayman et Texas sont abandonnés, chaque couleur devient
                    // naturelle et non forcing (5-6 cartes, jusqu'à 7HL). Vérifié : sans
                    // cette détection, le moteur continuait à tort avec les mécanismes
                    // Stayman/Texas standards (un "2♥" pouvait être un transfert vers
                    // 2♠ plutôt que du cœur naturel).
                    call = decideResponseToDoubledOneNT(hand, hcp, hl, seat, history);
                    explanation = `Réponse au contre du 1SA du partenaire (traité comme punitif) — système naturel, jamais Stayman/Texas (${points})`;
                } else {
        ...
        // Voir échange avec Guillaume (vraie donne testée, "pourquoi Sud dit 4SA ?") :
        // ma 1ère annonce était "2SA fitté" (convention déjà existante, fit EXACT 3
        // cartes en majeure, 10-11 points de soutien, voir decideRobotMajorSupport), et
        // le partenaire vient de signer simplement en manche dans cette majeure (aucun
        // contrôle/singleton montré, juste un "4♠" direct). J'ai déjà tout dit via mon
        // 2SA — rien de plus à ajouter, passe. Calculé ici, exclu explicitement des
        // branches suivantes plutôt que d'y être imbriqué (structure déjà complexe).
        const myFirstBidWasFittedTwoNTForSignoff = myBids[0] && myBids[0].call === '2NT';
        const partnerOpeningForFittedSignoff = history.find(e => partnershipOf(e.seat) === partnershipOf(seat)
            && e.seat !== seat && isBidCall(e.call));
        const partnerOpeningParsedForFittedSignoff = partnerOpeningForFittedSignoff ? parseBid(partnerOpeningForFittedSignoff.call) : null;
        const partnerRebidParsedForFittedSignoff = myPartnerBid ? parseBid(myPartnerBid.call) : null;
        const isFittedTwoNTThenMajorSignoff = myFirstBidWasFittedTwoNTForSignoff && partnerOpeningParsedForFittedSignoff
            && (partnerOpeningParsedForFittedSignoff.strain === 'S' || partnerOpeningParsedForFittedSignoff.strain === 'H')
            && partnerOpeningParsedForFittedSignoff.level === 1
            && partnerRebidParsedForFittedSignoff && partnerRebidParsedForFittedSignoff.strain === partnerOpeningParsedForFittedSignoff.strain
            && (partnerRebidParsedForFittedSignoff.level === 3 || partnerRebidParsedForFittedSignoff.level === 4);
        // Voir échange avec Guillaume (vraie donne testée, "pourquoi Sud revient à 3SA ?",
        // "le retour dans la majeure au palier minimal après un 2SA fitté montre une
        // main minimale... l'ouvreur qui revient à 3 dans sa couleur dit 'je suis
        // ultra-mini, laisse tomber' [même la manche]") : échelle à 3 niveaux précisée —
        // palier 3 = ultra-minimum (même pas la manche), palier 4 = accepte la manche,
        // au-delà = main encore plus forte. Les deux premiers niveaux se traitent pareil
        // ici (le répondant, ayant déjà tout dit via 2SA, accepte simplement ce que
        // l'ouvreur vient de préciser — jamais un repli SA générique).
        if (isFittedTwoNTThenMajorSignoff) {
            explanation = `2SA fitté déjà tout dit, le partenaire précise (${partnerRebidParsedForFittedSignoff.level}${STRAIN_SYMBOL[partnerRebidParsedForFittedSignoff.strain]}) — passe (${points})`;
        ...
        if (myBidWasResponseToPartnerDouble && myPartnerBid) {
            ...
            // Voir échange avec Guillaume (vraie donne testée, "pourquoi Est saute
            // brutalement à 5♦ ?") : bug trouvé — cette recherche ne vérifiait jamais si
            // l'annonce trouvée était une VRAIE ouverture (précédée uniquement de
            // passes) ou une simple INTERVENTION sur l'ouverture adverse. Une
            // intervention (comme "1♦" ici, sur le "1♣" adverse) n'a pas la même valeur
            // structurelle qu'une vraie ouverture pour tout ce qui suit (2/1 forcing,
            // zone de manche connue) — sans cette distinction, une réponse normale à une
            // simple intervention était traitée comme un 2/1 forcing de manche établi.
            const partnerOpeningWasRealOpening = partnerOpeningEntry
                && history.slice(0, history.indexOf(partnerOpeningEntry)).every(e => isPass(e.call));
            const partnerOpeningBid = (partnerOpeningEntry && partnerOpeningWasRealOpening) ? parseBid(partnerOpeningEntry.call) : null;
            ...
            const myResponseWasCuebidForKnowsGameZone = myResponseBid
                && history.some(e => partnershipOf(e.seat) !== partnershipOf(seat) && isBidCall(e.call)
                    && parseBid(e.call) && parseBid(e.call).strain === myResponseBid.strain);
            const knowsGameZone = partnerOpeningBid && myResponseBid && !myResponseWasCuebidForKnowsGameZone
            ...
            // Voir échange avec Guillaume (vraie donne testée, "pourquoi Sud dit 5♠ ?...
            // ce n'est pas logique de reparler quand la manche est atteinte, sauf si on
            // pense pouvoir aller au chelem") : bug trouvé — cette détection ne
            // distinguait pas un vrai 2/1 (palier 2, changement de couleur) d'un SPLINTER
            // (saut direct à 4-d'une-nouvelle-couleur après une ouverture MAJEURE) : leur
            // logique de points est fondamentalement différente (splinter = main faible
            // possible, fit déjà montré, chicane annoncée — jamais "13HL minimum" comme
            // un vrai 2/1). Un splinter mal reconnu comme 2/1 forçait à continuer au-delà
            // de la manche sans la moindre justification de chelem.
            const myResponseWasSplinterForCheck = myResponseBid && myResponseBid.level === 4
                && (partnerOpeningBid.strain === 'S' || partnerOpeningBid.strain === 'H') && partnerOpeningBid.level === 1;
            // Voir échange avec Guillaume (vraie donne testée, "pourquoi Ouest reparle ?") :
            // même famille que le splinter ci-dessus — un CUE-BID (ma réponse annonce la
            // couleur d'un ADVERSAIRE, pas une couleur naturelle à moi) est déjà une
            // information bornée et forcing de manche par elle-même (13+ points de
            // soutien) ; une fois acceptée par le partenaire au palier de manche, rien de
            // neuf à ajouter — jamais un vrai 2/1 ouvert nécessitant d'explorer plus loin.
            const myResponseWasCuebidForCheck = myResponseBid
                && history.some(e => partnershipOf(e.seat) !== partnershipOf(seat) && isBidCall(e.call)
                    && parseBid(e.call) && parseBid(e.call).strain === myResponseBid.strain);
            const was2over1GameForcing = partnerOpeningBid && myResponseBid && !myResponseWasSplinterForCheck && !myResponseWasCuebidForCheck
                && partnerOpeningBid.level === 1 && myResponseBid.level >= 2
                && myResponseBid.strain !== partnerOpeningBid.strain && myResponseBid.strain !== 'NT';

            // Voir échange avec Guillaume (vraie donne testée, "après un 2/1, les seules
            // enchères non forcing du répondant sont la 2SA ou le retour dans sa
            // couleur, pour montrer une main limite de 11HL, trop forte pour dire 1SA
            // initialement, mais pas assez pour imposer la manche") : exception
            // manquante au forcing automatique ci-dessus — avec ma propre couleur
            // longue (6+) et une main dans cette zone limite précise (10-11HL environ),
            // je peux revenir non forcing dans ma couleur au palier le plus bas
            // possible, plutôt que d'être poussé vers la zone de manche à tort.
            const responseLengthsForNFReturn = suitLengths(hand);
            const canReturnNonForcing = was2over1GameForcing && partnerRebidBid
                && partnerRebidBid.strain !== myResponseBid.strain
                && responseLengthsForNFReturn[myResponseBid.strain] >= 6
                && hl >= 10 && hl <= 11;
            ...
            if (partnerRebidWasLowNT && hl >= 18 && hl <= 20 && isHandBalancedForNT(responseLengths)
            ...
            } else if ((knowsGameZone || mustAnswerQuantitative || knowsGameZoneViaFit || (was2over1GameForcing && !canReturnNonForcing) || partnerRebidIsJumpToStrongNT) && partnerBidsCount === 2 && !isPartnerRaiseOfMyResponse && !myResponseWasCuebidForKnowsGameZone) {
                ...
                const myResponseWasCuebidForPreference = myResponseBid
                    && history.some(e => partnershipOf(e.seat) !== partnershipOf(seat) && isBidCall(e.call)
                        && parseBid(e.call) && parseBid(e.call).strain === myResponseBid.strain);
                if (partnerRebidBid.strain === partnerOpeningBid.strain && myResponseWasCuebidForPreference) {
                    // Voir échange avec Guillaume (vraie donne testée, "pourquoi Ouest
                    // reparle ?") : l'ouvreur a répété sa propre couleur — mais ça
                    // n'accepte automatiquement QUE si ma 1ère annonce était un cue-bid
                    // (déjà forcing de manche par elle-même, la répétition de l'ouvreur
                    // n'étant alors que la complétion naturelle attendue). Un vrai SAUT
                    // hors contexte cue-bid (comme 1♥-1♠-3♥) montre une force EXTRA et
                    // doit toujours inviter à continuer — pas la même main.
                    deliberatePassAfterTransfer = true;
                } else if (preferredSuit === partnerRebidBid.strain && hl <= 8) {
                    deliberatePassAfterTransfer = true;
        ...
        // Voir échange avec Guillaume (vraie donne testée, "3♦ me semble une enchère
        // d'essai, à la couleur plutôt qu'à 2SA... demandant au partenaire de revenir à
        // 3♠ s'il est mini, mettre 4♠ s'il est maxi, juger sa main s'il est entre les
        // deux") : construction complète — j'ai soutenu la majeure du partenaire (2ème
        // annonce), il redemande maintenant une NOUVELLE couleur (pas la majeure
        // convenue) SOUS le palier de manche : c'est une enchère d'essai naturelle.
        const myOpeningForTrial = myBids[0] && parseBid(myBids[0].call);
        const mySupportForTrial = myBids[1] && parseBid(myBids[1].call);
        const partnerFirstBidForTrial = history.find(e => partnershipOf(e.seat) === partnershipOf(seat)
            && e.seat !== seat && isBidCall(e.call));
        const partnerFirstParsedForTrial = partnerFirstBidForTrial ? parseBid(partnerFirstBidForTrial.call) : null;
        const partnerTrialBid = getLastActualBid(history);
        const partnerTrialParsed = partnerTrialBid ? parseBid(partnerTrialBid.call) : null;
        const isMajorSupportSequence = mySupportForTrial && partnerFirstParsedForTrial
            && (partnerFirstParsedForTrial.strain === 'S' || partnerFirstParsedForTrial.strain === 'H')
            && mySupportForTrial.strain === partnerFirstParsedForTrial.strain
            && mySupportForTrial.level === partnerFirstParsedForTrial.level + 1;
        const isTrialBid = isMajorSupportSequence && partnerTrialParsed
            && partnerTrialParsed.strain !== partnerFirstParsedForTrial.strain && partnerTrialParsed.strain !== 'NT'
            && partnerTrialParsed.level < 4
            && partnershipOf(partnerTrialBid.seat) === partnershipOf(seat) && partnerTrialBid.seat !== seat;
        if (isTrialBid) {
            const agreedMajor = partnerFirstParsedForTrial.strain;
            const trialSuit = partnerTrialParsed.strain;
            const gameCall = '4' + agreedMajor;
            const declineCall = '3' + agreedMajor;
            if (hcp >= 14) {
                if (isCallLegal(history, gameCall, seat)) {
                    call = gameCall;
                    explanation = `Maxi (14H+) après l'enchère d'essai à ${STRAIN_SYMBOL[trialSuit]} — manche dans le fit, jamais à SA (${points})`;
                }
            } else if (hcp <= 11) {
                if (isCallLegal(history, declineCall, seat)) {
                    call = declineCall;
                    explanation = `Mini (≤11H) après l'enchère d'essai à ${STRAIN_SYMBOL[trialSuit]} — décline, retour au palier minimal (${points})`;
                }
            } else {
                // Voir échange avec Guillaume ("s'il avait 13H dont la DK seconde... le
                // fait de savoir que ♦ est la couleur de son partenaire lui permettrait
                // de revaloriser sa main") : zone intermédiaire (12-13H) — jugement basé
                // sur une AIDE spécifique dans la couleur d'essai (honneur A/R/D, ou
                // chicane/singleton qui limite les pertes là-bas).
                const trialSuitCards = hand[trialSuit] || '';
                const hasHelpInTrialSuit = trialSuitCards.includes('A') || trialSuitCards.includes('K')
                    || trialSuitCards.includes('Q') || trialSuitCards.length <= 1;
                if (hasHelpInTrialSuit && isCallLegal(history, gameCall, seat)) {
                    call = gameCall;
                    explanation = `Zone intermédiaire (12-13H) mais aide réelle à ${STRAIN_SYMBOL[trialSuit]} (honneur ou chicane) — revalorise la main, manche dans le fit (${points})`;
                } else if (isCallLegal(history, declineCall, seat)) {
                    call = declineCall;
                    explanation = `Zone intermédiaire (12-13H) sans aide réelle à ${STRAIN_SYMBOL[trialSuit]} — décline, retour au palier minimal (${points})`;
                }
            }
        }

        // Voir échange avec Guillaume (vraie donne testée, "pourquoi Est ajoute 5♥ ?") :
        // cas manquant — ma 1ère annonce était un transfert Texas (2♦→♥ ou 2♥→♠) et ma
        // 2ème une invite "2SA" — le partenaire vient d'ACCEPTER clairement en manche
        // dans la majeure transférée (voir le fix symétrique côté ouvreur, "accepte
        // l'invite... vise la manche"). J'ai déjà tout dit (transfert + invite bornée),
        // rien de plus à ajouter — passe, jamais un repli générique qui pousse au-delà
        // de la manche sans justification.
        const myTransferAskForEast = myBids[0] && parseBid(myBids[0].call);
        const myInviteForEast = myBids[1] && myBids[1].call === '2NT';
        const transferredMajorForEast = myTransferAskForEast && myTransferAskForEast.level === 2
            ? (myTransferAskForEast.strain === 'D' ? 'H' : myTransferAskForEast.strain === 'H' ? 'S' : null)
            : null;
        const partnerAcceptForEast = getLastActualBid(history);
        const partnerAcceptParsedForEast = partnerAcceptForEast ? parseBid(partnerAcceptForEast.call) : null;
        if (myInviteForEast && transferredMajorForEast && partnerAcceptParsedForEast
                && partnerAcceptParsedForEast.strain === transferredMajorForEast && partnerAcceptParsedForEast.level === 4
                && partnershipOf(partnerAcceptForEast.seat) === partnershipOf(seat) && partnerAcceptForEast.seat !== seat) {
            explanation = `Transfert + invite déjà tout dit, le partenaire accepte en manche — passe (${points})`;
            deliberatePassAfterTransfer = true;
        }

        // Voir échange avec Guillaume (vraie donne testée, "pourquoi Sud reparle sur le
        // 2♥ [Coeur] de son partenaire ?") : cas manquant — ma REDEMANDE (pas mon
        // ouverture) était 1SA (12-14H typique, dénie 4 cartes dans la couleur du
        // répondant), et le partenaire revient simplement à SA PROPRE couleur au palier
        // minimal (non forcing, 5-6 cartes, main faible, rien d'autre à ajouter) — j'ai
        // déjà tout dit via mon 1SA, j'accepte, jamais un repli SA générique qui n'a
        // plus aucun sens une fois que le partenaire a clairement signé sa main faible.
        const myRebidWas1NTForSimpleReturn = myBids[1] && myBids[1].call === '1NT';
        const partnerFirstBidForSimpleReturn = history.find(e => partnershipOf(e.seat) === partnershipOf(seat)
            && e.seat !== seat && isBidCall(e.call));
        const partnerFirstParsedForSimpleReturn = partnerFirstBidForSimpleReturn ? parseBid(partnerFirstBidForSimpleReturn.call) : null;
        const partnerReturnForSimpleReturn = getLastActualBid(history);
        const partnerReturnParsedForSimpleReturn = partnerReturnForSimpleReturn ? parseBid(partnerReturnForSimpleReturn.call) : null;
        if (myRebidWas1NTForSimpleReturn && partnerFirstParsedForSimpleReturn
                && (partnerFirstParsedForSimpleReturn.strain === 'S' || partnerFirstParsedForSimpleReturn.strain === 'H')
                && partnerReturnParsedForSimpleReturn && partnerReturnParsedForSimpleReturn.strain === partnerFirstParsedForSimpleReturn.strain
                && partnerReturnParsedForSimpleReturn.level === 2
                && partnershipOf(partnerReturnForSimpleReturn.seat) === partnershipOf(seat) && partnerReturnForSimpleReturn.seat !== seat) {
            explanation = `Ma redemande 1SA a déjà tout dit, le partenaire revient à sa couleur au minimum — j'accepte (${points})`;
            deliberatePassAfterTransfer = true;
        }

        // Voir échange avec Guillaume (vraie donne testée, "Ouest n'a pas à dire 4SA") :
        // cas manquant — mon partenaire vient de RECONVERTIR vers sa propre couleur
        // (majeure) après que j'ai moi-même annoncé du SA à mes 2 tours précédents —
        // signal de force réelle là-bas (voir le fix symétrique côté ouvreur, "ma
        // couleur l'emporte sur le SA du partenaire"). Sans soutien réel ni réserve
        // chez moi, j'accepte simplement, jamais un repli SA générique qui n'a plus
        // aucun sens une fois l'atout ainsi reconfirmé.
        const myLastBidWasNTForReconvert = myBids[1] && myBids[1].call.endsWith('NT');
        const partnerJustReconvertedToMajor = getLastActualBid(history);
        const partnerReconvertParsed = partnerJustReconvertedToMajor ? parseBid(partnerJustReconvertedToMajor.call) : null;
        if (myLastBidWasNTForReconvert && partnerReconvertParsed
                && (partnerReconvertParsed.strain === 'S' || partnerReconvertParsed.strain === 'H')
                && partnerReconvertParsed.level === 4
                && partnershipOf(partnerJustReconvertedToMajor.seat) === partnershipOf(seat) && partnerJustReconvertedToMajor.seat !== seat
                && suitLengths(hand)[partnerReconvertParsed.strain] <= 2 && hcp <= 11) {
            explanation = `Le partenaire reconvertit vers sa couleur après mon SA — j'accepte, rien à ajouter (${points})`;
            deliberatePassAfterTransfer = true;
        }

        // Voir échange avec Guillaume (vraie donne testée, "le fit à 3♠ de Sud est
        // forcing") : cas manquant — le répondant a fait un vrai 2/1 (forcing de manche
        // établi dès ce moment, confirmé avec Guillaume donne 5), j'ai redemandé une
        // nouvelle couleur, et il revient maintenant SOUTENIR ma couleur d'OUVERTURE —
        // toujours forcing dans ce contexte (le 2/1 reste forcing jusqu'à la manche,
        // sauf 2SA ou retour dans SA PROPRE couleur du répondant, jamais un simple
        // soutien de la mienne). Sans ce garde-fou, ça tombait sur le filet "déjà
        // annoncé, passe" par défaut, même avec une main bien au-dessus du minimum.
        const myOpeningForRaiseCheck = myBids[0];
        const myOpeningParsedForRaiseCheck = parseBid(myOpeningForRaiseCheck.call);
        const myOpeningWasRealForRaiseCheck = history.slice(0, history.indexOf(myOpeningForRaiseCheck)).every(e => isPass(e.call));
        const lastRealBidForRaiseCheck = getLastActualBid(history);
        const partnerFirstRealBidForRaiseCheck = history.find(e => partnershipOf(e.seat) === partnershipOf(seat)
            && e.seat !== seat && isBidCall(e.call));
        const partnerFirstParsedForRaiseCheck = partnerFirstRealBidForRaiseCheck ? parseBid(partnerFirstRealBidForRaiseCheck.call) : null;
        const wasGenuine2Over1ForRaiseCheck = myOpeningParsedForRaiseCheck && myOpeningParsedForRaiseCheck.level === 1
            && (myOpeningParsedForRaiseCheck.strain === 'S' || myOpeningParsedForRaiseCheck.strain === 'H')
            && partnerFirstParsedForRaiseCheck && partnerFirstParsedForRaiseCheck.level >= 2
            && partnerFirstParsedForRaiseCheck.strain !== myOpeningParsedForRaiseCheck.strain && partnerFirstParsedForRaiseCheck.strain !== 'NT';
        // Voir échange avec Guillaume (correction précise après une régression, "après un
        // 2/1, une préférence montre 2 cartes et 11HL... s'il dit 3♠ [sans sauter] c'est
        // qu'il a aussi un espoir de chelem [seulement] si ma redemande promettait déjà
        // 15H+ [nouvelle couleur au palier 3]") : le facteur déterminant n'est pas
        // simplement "2/1 en cours", mais si MA PROPRE redemande promettait déjà 15H+ —
        // sans ça, le retour du répondant sans saut n'est qu'une simple préférence
        // plafonnée (2 cartes, 11HL), pas un signal de chelem à explorer.
        const myRebidPromised15PlusForRaiseCheck = myBids[1] && parseBid(myBids[1].call)
            && parseBid(myBids[1].call).strain !== myOpeningParsedForRaiseCheck?.strain
            && parseBid(myBids[1].call).strain !== 'NT' && parseBid(myBids[1].call).level === 3;
        if (myOpeningWasRealForRaiseCheck && wasGenuine2Over1ForRaiseCheck && myRebidPromised15PlusForRaiseCheck && lastRealBidForRaiseCheck
                && lastRealBidForRaiseCheck.call !== myOpeningForRaiseCheck.call
                && parseBid(lastRealBidForRaiseCheck.call) && parseBid(lastRealBidForRaiseCheck.call).strain === myOpeningParsedForRaiseCheck.strain
                && partnershipOf(lastRealBidForRaiseCheck.seat) === partnershipOf(seat) && lastRealBidForRaiseCheck.seat !== seat) {
            const supportPointsForRaiseCheck = computeSupportPoints(hand, myOpeningParsedForRaiseCheck.strain, 3);
            const gameLevelForRaiseCheck = 4;
            const lastLevelForRaiseCheck = parseBid(lastRealBidForRaiseCheck.call).level;
            let targetLevelForRaiseCheck = Math.max(gameLevelForRaiseCheck, lastLevelForRaiseCheck);
            if (supportPointsForRaiseCheck + 13 >= SLAM_ZONE_GRAND) targetLevelForRaiseCheck = 7;
            else if (supportPointsForRaiseCheck + 13 >= SLAM_ZONE_SMALL) targetLevelForRaiseCheck = 6;
            for (let level = targetLevelForRaiseCheck; level <= 7; level++) {
                const c = level + myOpeningParsedForRaiseCheck.strain;
                if (isCallLegal(history, c, seat)) {
                    call = c;
                    explanation = `Ma redemande (palier 3, nouvelle couleur) promettait déjà 15H+ — le retour du partenaire sans saut montre un espoir de chelem, jamais un simple passe (${points})`;
                    break;
                }
            }
        }
        ...
        // Voir échange avec Guillaume (vraie donne testée, "Ouest étant fitté à ♥...
        // il doit rectifier à 3♥... il a 17HL, donc pourquoi n'accepte-t-il pas ?") :
        // cas manquant précisément — vraie ouverture 1SA (pas CRM), complétée par un
        // transfert majeur normal (ma 2ème enchère), le répondant invite maintenant
        // par 2SA. Le fit majeur (5+ chez lui, 2+ chez moi pour avoir accepté) est
        // DÉJÀ connu — accepter doit viser la manche DANS la majeure (pas 3SA),
        // décliner doit rectifier vers la couleur (jamais rester en SA, contrairement
        // à une invite SA directe classique).
        if (myBids0WasRealOpening && myBids[0].call === '1NT' && myBids[1] && myBids.length === 2) {
            const myTransferRebid = parseBid(myBids[1].call);
            const transferredMajor = myTransferRebid && myTransferRebid.level === 2
                ? (myTransferRebid.strain === 'H' ? 'H' : myTransferRebid.strain === 'S' ? 'S' : null)
                : null;
            const lastRealBidForTransferInvite = getLastActualBid(history);
            if (transferredMajor && lastRealBidForTransferInvite && lastRealBidForTransferInvite.call === '2NT'
                    && partnershipOf(lastRealBidForTransferInvite.seat) === partnershipOf(seat) && lastRealBidForTransferInvite.seat !== seat) {
                if (hl >= 17) {
                    const c = '4' + transferredMajor;
                    if (isCallLegal(history, c, seat)) {
                        return { call: c, explanation: `Accepte l'invite du partenaire — fit majeur déjà connu (${transferredMajor === 'H' ? '♥' : '♠'}), vise la manche là plutôt qu'à SA (${points})` };
                    }
                } else {
                    const supportLenForTransferInvite = suitLengths(hand)[transferredMajor];
                    if (supportLenForTransferInvite <= 2) {
                        const c = '3' + transferredMajor;
                        if (isCallLegal(history, c, seat)) {
                            return { call: c, explanation: `Décline l'invite mais rectifie vers le fit majeur déjà connu (${transferredMajor === 'H' ? '♥' : '♠'}) — support minimal (2 cartes), jamais rester à SA (${points})` };
                        }
                    }
                }
            }
