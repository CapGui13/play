...
Ça vaut aussi pour `donnes/catalogue.json` et les fichiers de `donnes/` : ils ne sont pas
dans `CORE_ASSETS` (mis en cache seulement après un premier chargement réussi, pas
pré-téléchargés à l'installation), mais restent soumis à la même stratégie "cache
d'abord" une fois chargés — voir `donnes/README.md`.
